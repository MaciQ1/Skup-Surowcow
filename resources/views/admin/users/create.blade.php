            ...
            <div class="col-6">
                <form method="POST" action="{{ route('admin.users.store') }}" class="needs-validation" novalidate>
                    ...
                    <div class="form-group mb-2">
                        ...
                        <input id="password" name="password" type="password" min="1" class="form-control @if ($errors->first('password')) is-invalid @endif" value="{{ old('password') }}">
                        <div class="invalid-feedback">Nieprawidłowe hasło!</div>
                    </div>
                    <div class="form-group mb-2">
                        <label for="address" class="form-label">Adres</label>
                        <input id="address" name="address" type="text" class="form-control @if ($errors->first('address')) is-invalid @endif" value="{{ old('address') }}">
                        <div class="invalid-feedback">Nieprawidłowy adres!</div>
                    </div>
                    <div class="form-group mb-2">
                        <label for="phone_number" class="form-label">Numer telefonu</label>
                        <input id="phone_number" name="phone_number" type="number" max="9" class="form-control @if ($errors->first('phone_number')) is-invalid @endif" value="{{ old('phone_number') }}">
